...
#' @param ring_label_size The size of labels on the ring chart.
...
pav_hist <- function(pav_obj,
  ...
  Var1 = Freq = type = x = per = NULL

  p_data <- merge(data.frame(table(regions_data$sample_n)),
  ...
  if(show_ring){
    ...
    ring_data <- data.frame(table(regions_data$type)[types])
    ring_data <- ring_data[!is.na(ring_data$Freq),]
    ring_data$x <- ring_data$Freq/2 + utils::head(c(0, cumsum(ring_data$Freq)), -1)
