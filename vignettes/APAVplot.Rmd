...
* The input is the output table from the `APAV sim` tool.
* The `sim_plot()` function draws growth curves for genome simulation.
...
The target regions can be categorized into multiple types based on the number of samples containing them. The function `pav_hist()` integrates a ring chart and a histogram to display the number of types. The parameters `ring_pos_x`, `ring_pos_y`, and `ring_r` specify the position and radius of the ring chart. The x-axis of the histogram represents the number of samples, ranging from 1 to the total number of samples. The y-axis indicates the number of regions shared by `x` samples.
...
The function `pav_heatmap()` generates a heatmap that includes two summary annotations. The columns are split into blocks based on region types. If the parameter `split_block` is set to `FALSE`, the split line will be removed.
...
A simulation can effectively estimate genome size, and this can be achieved through the `APAV sim` tool, which generates essential input simulation results. The `plot_sim` function offers three distinct chart types: "jitter," "ribbon," and "errorbar," allowing for versatile visual representations of the data.

```{r fig.height=4, fig.width=6}
data("sim_res")
plot_sim(sim_res, y_title = "Gene Number")
plot_sim(sim_res, chart_type = "jitter", y_title = "Gene Number")
plot_sim(sim_res, chart_type = "errorbar", y_title = "Gene Number")
```
...
```{r fig.height=4, fig.width=6}
plot_sim(sim_res, data_type = "increasing", 
         y_title = "Number of the increasing gene")
plot_sim(sim_res, data_type = "increasing", chart_type = "jitter", 
         y_title = "Number of the increasing gene")
plot_sim(sim_res, data_type = "increasing", chart_type = "errorbar", 
         y_title = "Number of the increasing gene")
```
...
When working with grouped samples, the `sim_plot` function can be employed to create individualized growth curves for each group, facilitating a more nuanced analysis.
...
plot_ele_depth(ele_depth, ele_cov, pheno_data = ele_pheno)
```
